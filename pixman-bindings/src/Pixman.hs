...
import qualified Pixman.Generated as G
import qualified Pixman.Generated.FunPtr as FP
import qualified Pixman.Generated.Unsafe as US
...
data family Region (a :: RegionType) :: Type

data RegionType = R16 | R32 | R64F
  deriving (Eq, Ord, Show, Read, Generic)

newtype instance Region 'R16  = Region16  { unwrap :: ForeignPtr G.Pixman_region16_t  } deriving stock (Ord, Generic)
newtype instance Region 'R32  = Region32  { unwrap :: ForeignPtr G.Pixman_region32_t  } deriving stock (Ord, Generic)
newtype instance Region 'R64F = Region64F { unwrap :: ForeignPtr G.Pixman_region64f_t } deriving stock (Ord, Generic)

---- | 16-bit region
type Region16 = Region 'R16

---- | 32-bit region
type Region32 = Region 'R32

---- | 64-bit (floating-point) region
type Region64F = Region 'R64F

instance (IsRegion (Region a), Typeable a, Show (RBox (Region a))) => Show (Region a) where
  show reg = printf "Region[%s]{extents=%s, numRects=%d, rects=%s}" (show $ typeRep (Proxy @a)) (show ext) numRects (show rects)
    where ext = regionExtents reg
          numRects = regionNumRects reg
          rects = regionRectangles numRects reg

instance IsRegion (Region a) => Eq (Region a) where
  (==) = regionEqual

-- | Map region types to corresponding box types
...
instance Show Box16 where show box = "Box16{" ++ show [getField @"x1" box, getField @"y1" box, getField @"x2" box, getField @"y2" box] ++ "}"
instance Show Box32 where show box = "Box32{" ++ show [getField @"x1" box, getField @"y1" box, getField @"x2" box, getField @"y2" box] ++ "}"
instance Show Box64F where show box = "Box64F{" ++ show [getField @"x1" box, getField @"y1" box, getField @"x2" box, getField @"y2" box] ++ "}"
...
instance Show Rectangle16 where show x = printf "Rectangle16{x=%d,y=%d,width=%d,height=%d}" (getField @"x" x) (getField @"y" x) (getField @"width" x) (getField @"height" x)
instance Show Rectangle32 where show x = printf "Rectangle32{x=%d,y=%d,width=%d,height=%d}" (getField @"x" x) (getField @"y" x) (getField @"width" x) (getField @"height" x)
instance Show Rectangle64F where show x = printf "Rectangle64F{x=%f,y=%f,width=%f,height=%f}" (getField @"x" x) (getField @"y" x) (getField @"width" x) (getField @"height" x)
...
imageSetClipRegion :: (img ~ G.Pixman_image_t, HasField "unwrap" reg (ForeignPtr r))
                   => String
                   -> (Ptr img -> ConstPtr r -> IO G.Pixman_bool_t)
                   -> reg -> CreateImage -> CreateImage
...
data SetComposite (a :: RegionType) = SetComposite
  { op       :: Op
  , srcImg   :: IO Image
  , maskImg  :: Maybe Image
  , srcPos   :: XY (PosType a)
  , maskPos  :: XY (PosType a)
  , destPos  :: XY (PosType a)
  , size     :: Size2D (DimType a)
  }

type family PosType (a :: RegionType) :: Type
type instance PosType R16 = Int16
type instance PosType R32 = Int32
type instance PosType R64F = Double

type family DimType (a :: RegionType) :: Type
type instance DimType R16 = Word16
type instance DimType R32 = Int32
type instance DimType R64F = Double

imageSetComposite :: _ -> (PosType a -> _) -> (DimType a -> _) -> SetComposite a -> CreateImage -> CreateImage
...
imageSetComposite16 :: SetComposite R16 -> CreateImage -> CreateImage
imageSetComposite16 = imageSetComposite US.pixman_image_composite id id

imageSetComposite32 :: SetComposite R32 -> CreateImage -> CreateImage
imageSetComposite32 = imageSetComposite US.pixman_image_composite32 id id

imageSetComposite64F :: SetComposite R64F -> CreateImage -> CreateImage
imageSetComposite64F = imageSetComposite US.pixman_image_composite64f CDouble CDouble

setComposite :: (Default (PosType a), Default (DimType a), IsImage src) => src -> SetComposite a
setComposite src = SetComposite OpSrc (toImage src) Nothing (def, def) (def, def) (def, def) (def, def)

setComposite16 :: IsImage src => src -> SetComposite R16
setComposite16 = setComposite

setComposite32 :: IsImage src => src -> SetComposite R32
setComposite32 = setComposite

setComposite64F :: IsImage src => src -> SetComposite R64F
...
withNewForeignPtr
  :: Storable a => (ForeignPtr a -> IO b) -> (Ptr a -> IO ()) -> IO b
withNewForeignPtr f g = do
  fp <- mallocForeignPtr
  withForeignPtr fp g
  f fp

withNewForeignPtrCopy
  :: (Storable a, HasField "unwrap" a' (ForeignPtr a))
  => (ForeignPtr a -> IO b) -> a' -> (Ptr a -> IO ()) -> IO b
