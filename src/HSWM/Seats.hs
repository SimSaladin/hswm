...
handleXkbBindingsSeatEvent ev = case ev of
  ...
  R.RiverXkbBindingsSeatAteUnboundKey dt _ ->
    modifySeat' dt $ \s -> s { pending_action = S_SUBMAP_CANCEL }
  ...
  where
    ...
    managePendingAction = case s.pending_action of
      ...
      S_SUBMAP_NEXT_KEY action subkeys -> do
        ...
        io $ mapM_ (deRefStablePtr >=> R.riverXkbBindingDisable . xkb_binding) $
...
createSeatBindings s = do
  ...
  kPtrs <- createXkbBindings (binds, kbdListen, s.river_seat) actionSubmap
...
execXkbBinding xb = withSeat xb.river_seat $ \s -> do
  case (s.submap_pending, actionSubmap xb.action) of
    ...
    (Nothing, _) -> do
      modifySeat xb.river_seat $ \s' -> s' { pending_action = S_SUBMAP_NEXT_KEY xb.action xb.subKeymap }
    -- Submap action + reset
    (Just _, []) -> do
