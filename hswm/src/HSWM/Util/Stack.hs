...
fromTags = finalize . foldr step ([], Nothing, [])
  where
    step (Right a) (u, Just f, d) = ([], Just a, u ++ f : d)
...
-- as 'HSWM.StackSet.differentiate' if @zs@ and @xs@ don't intersect.
...
focusMasterZ (Just (W.Stack f up down))
  | not $ null up =
...
findS p st = st <$ (guard . p . W.focus) st <|> findUp st <|> findDown st
  where
    findDown = reverseZ . findUp . reverseS
    findUp s
      | u : ups <- W.up s =
          (if p u then Just else findUp) $
...
sortByZ f = fromTags . sortBy (adapt f) . toTags
  where
    adapt g e1 e2 = g (fromE e1) (fromE e2)
...
-- mimics HSWM's usual one. The boolean argument indicates whether the current
...
deleteIndexZ i z =
  let numbered = (fromTags . zipWith number [0 ..] . toTags) z
...
foldrZ f b (Just s) =
  let b1 = foldr (f False) b (W.down s)
...
foldlZ f b (Just s) =
  let b1 = foldr (flip $ f False) b (W.up s)
...
getI :: Int -> [a] -> Maybe a
getI i xs = xs !? i
{-# DEPRECATED getI "Use HSWM.Prelude.(!?) instead." #-}
