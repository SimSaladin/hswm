...
import qualified HSWM.Util.Rectangle          as R
...
noBorders :: LayoutClass l RiverWindow => l RiverWindow -> ModifiedLayout WithBorder l RiverWindow
...
setBorders :: [RiverWindow] -> Dimension -> HS ()
setBorders ws bw = forM_ ws $ \w -> manageWindowBorderWidth w (Just $ fi bw)
...
hasBorder b = ask >>= \w -> liftHS (broadcastMessage $ HasBorder b w.river_window) >> idHook

data BorderMessage
    = HasBorder Bool RiverWindow
        -- ^ If @True@, never remove the border from the specified window. If
        -- @False@, always remove the border from the specified window.
    | ResetBorder RiverWindow
...
class SetsAmbiguous p where
    hiddens :: p -> WindowSet -> Rectangle -> Maybe (W.Stack RiverWindow) -> [(RiverWindow, Rectangle)] -> [RiverWindow]
...
--   Given by 'HSWM.Core.LayoutClass' where earlier entries are stacked
--   higher.
...
--
-- 'HSWM.Core.LayoutClass' is given a stack with all floating windows removed
...
--
-- 'HSWM.Operations.windows' gets the windowset from the state, mutates it,
...
instance SetsAmbiguous Ambiguity where
    hiddens amb wset lr mst wrs
      ...
      where next p = hiddens p wset lr mst wrs
            ...
            floating = do
                scr <- thisScreen
                let wz :: Integer -> (RiverWindow,Rectangle)
                       -> (Integer,RiverWindow,Rectangle)
                    wz i (w,wr) = (i,w,wr)
