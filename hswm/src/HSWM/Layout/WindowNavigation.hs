...
import Data.List ( nub, sortBy, (\\), repeat )
...
data Navigate = Go Direction2D | Swap Direction2D | Move Direction2D
              | Apply (RiverWindow -> H ()) Direction2D -- ^ Apply action with destination window
...
instance LayoutModifier WindowNavigation RiverWindow where
    redoLayout (WindowNavigation conf (I st)) rscr (Just s) origwrs =
        do HSWMConfig { normalBorder = nbc, focusedBorder = fbc } <- asks config
    ...
    handleMessOrMaybeModifyIt (WindowNavigation conf (I (Just (NS pt wrs)))) m
        | Just (Go d) <- fromMessage m =
                         ...
                         ((w,r):_) -> do modify focusWindowHere
                             ...
                             where focusWindowHere :: HState -> HState
                                   focusWindowHere s
        ...
        | Just Hide <- fromMessage m =
                       do HSWMConfig { normalBorder = nbc } <- asks config
