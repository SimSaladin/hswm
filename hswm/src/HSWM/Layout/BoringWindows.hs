...
-- |
-- Module      :  HSWM.Layout.BoringWindows
...
import Data.List (find, iterate, union, (\\))
import Data.List.NonEmpty qualified as NE
import Data.Map qualified as M
import HSWM
import HSWM.Layout.LayoutModifier (LayoutModifier (handleMessOrMaybeModifyIt, redoLayout), ModifiedLayout (..))
import HSWM.StackSet qualified as W
import HSWM.Util.Stack (reverseS)

-- $usage
-- You can use this module with the following in your
-- @xmonad.hs@:
--
-- > import HSWM.Layout.BoringWindows
--
...
data BoringMessage
  ...
  | FocusDown
  | FocusMaster
  | IsBoring RiverWindow
  | ClearBoring
  | Replace String [RiverWindow]
  | Merge String [RiverWindow]
  | SwapUp
  | SwapDown
  | SiftUp
  | SiftDown
...
instance LayoutModifier BoringWindows RiverWindow where
  ...
  handleMessOrMaybeModifyIt bst@(BoringWindows nbs cbs lbs) m
    ...
    | Just (IsBoring w) <- fromMessage m,
      w `notElem` cbs =
        rjl bst {chosenBoring = w : cbs}
    | Just ClearBoring <- fromMessage m,
      not (null cbs) =
        rjl bst {namedBoring = M.empty, chosenBoring = []}
    | Just FocusUp <- fromMessage m =
        do
          windows $ W.modify' $ skipBoring W.focusUp'
          return Nothing
    | Just FocusDown <- fromMessage m =
        do
          windows $ W.modify' $ skipBoring W.focusDown'
          return Nothing
    ...
    | Just SwapUp <- fromMessage m =
        do
          windows $ W.modify' skipBoringSwapUp
          return Nothing
    | Just SwapDown <- fromMessage m =
        do
          windows $ W.modify' (reverseS . skipBoringSwapUp . reverseS)
          return Nothing
    | Just SiftUp <- fromMessage m =
        do
          windows $ W.modify' (siftUpSkipping bs)
          return Nothing
    | Just SiftDown <- fromMessage m =
        do
          windows $ W.modify' (reverseS . siftUpSkipping bs . reverseS)
          return Nothing
    where
      skipBoring = skipBoring' ((`notElem` bs) . W.focus)
      skipBoringSwapUp =
        skipBoring'
...
-- $simplest
--
-- An alternative to 'Full' is "HSWM.Layout.Simplest".  Less windows are
