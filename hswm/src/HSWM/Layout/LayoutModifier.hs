...
--
-- * "HSWM.Layout.WorkspaceDir"
--
-- * "HSWM.Layout.Magnifier"
--
...
class (Show (m a), Read (m a)) => LayoutModifier m a where
    ...
    --   perform some effect in the HS monad, and\/or modify some of
    ...
    modifyLayout :: (HandleLayouts menv, LayoutClass l a) =>
                    m a                             -- ^ the layout modifier
                 -> Workspace WorkspaceId (l a) a WorkspaceDetail   -- ^ current workspace
    ...
    -- 'redoLayout' as well.
    modifyLayoutWithUpdate :: (HandleLayouts menv, LayoutClass l a) =>
                              m a
    ...
    --   layout, in order to have an effect in the HS monad, or alter
    ...
    --   modify messages or have access to the HS monad, you should use
    --   'pureMess' instead.
    --
    ...
    --   the HS monad, alter the layout modifier state, or produce a
    ...
    --   HS monad, possibly return a new layout modifier, and\/or
    --   modify the results of 'runLayout' before returning them.
    --
    --   If you don't need access to the HS monad, use 'pureModifier'
    --   instead.  Also, if the behavior you need can be cleanly
    --   separated into an effect in the HS monad, followed by a pure
    ...
    --   'redoLayout', and as such represents an HS action which is to
    ...
    --   of a "HSWM.Hooks.StatusBar" user).
...
instance (LayoutModifier m a, LayoutClass l a, Typeable m) => LayoutClass (ModifiedLayout m l) a where
    ...
    runLayout (Workspace i (ModifiedLayout m l) ms wd) r =
        do ((ws, ml'),mm')  <- modifyLayoutWithUpdate m (Workspace i l ms wd) r
