...
setFocus FocusCurrent old new =
  case lookupWorkspace (W.screen $ current old) new of
    Nothing -> new
    Just i -> W.view i new
...
viewOnScreen ::
  ...
  WindowSet
viewOnScreen sid i =
  onScreen (W.view i) (FocusTag i) sid
...
onlyOnScreen ::
  ...
  WindowSet
onlyOnScreen sid i =
  onScreen (W.view i) FocusCurrent sid

-- | @toggleOrView@ as in "HSWM.Actions.CycleWS" for @onScreen@ with view
toggleOnScreen ::
  -- | screen id
  ScreenId ->
  -- | index of the workspace
...
toggleOnScreen sid i =
  onScreen (toggleOrView' W.view i) FocusCurrent sid

-- | @toggleOrView@ from "HSWM.Actions.CycleWS" for @onScreen@ with greedyView
toggleGreedyOnScreen ::
  -- | screen id
  ScreenId ->
  -- | index of the workspace
