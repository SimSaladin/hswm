...
-- @
mkWindowNavigationKeys ::
  -- | keys for (Up, Left, Down, Right)
  (key, key, key, key) ->
  -- | mask for (go-to-window, swap-window)
  (mask, mask) ->
  -- | how to combine keys with masks
  (mask -> key -> a) ->
  -- | (keyBindings, logHook)
  IO ([(a, SomeAction H)], H ())
mkWindowNavigationKeys (u, l, d, r) (modGo, modSwap) f = do
  ...
  return
    ( [ (f modGo u, fromWNAction stateRef $ WNGo U),
        ...
        (f modSwap r, fromWNAction stateRef $ WNSwap R)
      ],
...
swapPure = withTargetWindow swapWithFocused
  where
    swapWithFocused targetWin winSet =
      case W.peek winSet of
        Just currentWin ->
          W.focusWindow currentWin $
...
navigableTargets input@(_, oldWindowSet, _, _) dir currentRect currentPos = do
  ...
  let screenWindows = S.fromList $ W.integrate' $ W.stack $ W.workspace $ W.current oldWindowSet
      ...
      rectInside r =
        (rect_p1 r >= rect_p1 wr && rect_p1 r < rect_p2 wr && rect_p2 r > rect_p1 wr && rect_p2 r <= rect_p2 wr)
      ...
      rectOverlapsEdge r =
        rect_p1 r <= rect_p2 wr
          && rect_p2 r > rect_p2 wr
          && rect_o1 r < rect_o2 wr
          && rect_o2 r > rect_o1 wr
      rectOverlapsOneEdge r = rectOverlapsEdge r && rect_p1 r > rect_p1 wr
      rectOverlapsBothEdges r =
        rectOverlapsEdge r
          && rect_o1 r > rect_o1 wr
          && rect_o2 r < rect_o2 wr
          && point_o pos >= rect_o1 r
          && point_o pos < rect_o2 r
  ...
  return $
    mapSnd (inversePointTransform dir) $
      concat
        [ -- First, navigate to windows that are fully inside current window
          ...
          mapSnd (\r -> DirPoint (rect_p2 r - 1) (clamp (point_o pos) (rect_o1 r) (rect_o2 r))) $
            sortByP2 $
              filterSnd (not . posBeforeEdge) $
                filterSnd rectInside thisScreenWindowsAndRectangles
        ]
...
windowRects (_, oldWindowSet, mappedWindows, windowRect) =
  let allWindows = filter (\w -> w `notElem` W.peek oldWindowSet) $ S.toList mappedWindows
