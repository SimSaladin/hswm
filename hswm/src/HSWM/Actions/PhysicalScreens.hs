...
-- $usage
--
-- This module allows you name Xinerama screens from HSWM using their
...
-- your H server and graphics hardware.
--
...
--
-- > import HSWM.Actions.PhysicalScreens
-- > import Data.Default
--
...
getScreen (ScreenComparator cmpScreen) (P i) = do
  ...
  if i < 0 || i >= length screens
    then return Nothing
      ...
      let ss = sortBy (cmpScreen `on` getScreenIdAndRectangle) screens
...
screenComparatorByRectangle rectComparator = ScreenComparator comparator
  where
...
screenComparatorById idComparator = ScreenComparator comparator
  where
...
verticalScreenOrderer = screenComparatorByRectangle comparator
  where
...
horizontalScreenOrderer = screenComparatorByRectangle comparator
  where
...
--  -- the 'HSWM.Operations.rescreen' implementation from core as a fallback
